...
public:
  ...
  static GW::GW_Bool FastMarchingStopCallback(
    ...
    filter->NumberOfVisitedPoints++;

    if (filter->MinimumNumberOfPointsToVisit > 0 &&
        filter->MinimumNumberOfPointsToVisit < filter->NumberOfVisitedPoints)
      {
      return false;
      }
    ...
    if (filter->DistanceStopCriterion > 0)
      {
      if (filter->DistanceStopCriterion <= v.GetDistance())
        {
        return true;
        }
      }
    ...
    */
    ...
    //Stop if k vertices have been visited
    if  (filter->NumberOfVisitedPointsStopCriterion > 0)
      {
      //cout << filter->NumberOfVisitedPoints << endl;
      if (filter->NumberOfVisitedPointsStopCriterion <= filter->NumberOfVisitedPoints)
        {
        return true;
        }
...
vtkFastMarchingGeodesicDistance::vtkFastMarchingGeodesicDistance()
{
  ...
  this->NumberOfVisitedPointsStopCriterion = -1;
...
void vtkFastMarchingGeodesicDistance::SetupCallbacks()
{
  ...
  if (this->DistanceStopCriterion > 0 ||
      this->NumberOfVisitedPointsStopCriterion > 0 ||

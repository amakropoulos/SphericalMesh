...
public:
  ...
  // Description:
  // Optionally stopping criteria may be specified. This method may be used to
  // restrict fast marching to k visted points from the seed(s). The
  // default is -1 which implies no stopping criteria.
  vtkSetMacro(NumberOfVisitedPointsStopCriterion, int );
  vtkGetMacro(NumberOfVisitedPointsStopCriterion, int );

  // Description:
  // Set lower bound on number of points to visit before termination.
  // Can be used in combination with a distance stopping criterion, for example.
  vtkSetMacro(MinimumNumberOfPointsToVisit, int );
  vtkGetMacro(MinimumNumberOfPointsToVisit, int );
...
protected:
  ...
  // Number of visited points stop criterion
  int NumberOfVisitedPointsStopCriterion;

  // Number of visited points stop criterion
  int MinimumNumberOfPointsToVisit;

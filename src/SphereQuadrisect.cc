...
SphereQuadrisect::SphereQuadrisect(){
}
...
typedef std::map<edge, int, edgeCmp> midpointMap;

std::map<edge, short>::iterator iter;
...
vtkSmartPointer<vtkPolyData> SphereQuadrisect::Quadrisect(vtkSmartPointer<vtkPolyData> input, int levels) {
...
//    copyFaces(newFaces, oldFaces);
    ...
    for(i = 0; i < noOfFacesOld; ++i){
      ...
      */
      ...
      e.id0 = std::min(id0, id2);
      e.id1 = std::max(id0, id2);
      ida = midptMap[e];

      e.id0 = std::min(id0, id1);
      e.id1 = std::max(id0, id1);
      idb = midptMap[e];

      e.id0 = std::min(id1, id2);
      e.id1 = std::max(id1, id2);
      idc = midptMap[e];
